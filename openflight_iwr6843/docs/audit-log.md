...
| D-5 | ✔ verified | Physical input (chirp) | Geometry | golf.cfg is legal per the chip datasheet (slope/sample-rate/IF/RF-band all inside limits) |
| D-6 | **MED** | Physical input (ADC) | Spin | The HiFiBerry sheet in the folder is the **wrong product** (DAC2 Pro, playback-only) — our board's sheet is missing |
