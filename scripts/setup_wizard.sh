...
#   9. systemd auto-start unit (optional; walk-up-and-swing on power-on)
#
...
# PINNED upstream commit: the exact tree the patches/ stack and the LM
...
if [ -d "$UPSTREAM_DIR/ui" ]; then
    ...
    # Pi would serve whatever stale dist/ exists -- i.e. NOT the LM reskin
...
# ---- 9. systemd auto-start unit (optional) --------------------------------

# Walk-up-and-swing: run_iwr6843.py comes up by itself on power-on, no SSH
# (TODO "Auto-start OpenFlight on Pi boot"). The unit execs the repo venv's
# python directly (no shell), reads ports/audio from hardware.env like any
# manual run, and restarts on FAILURE only -- note the S-1 dead-UART guard
# ends acquisition loudly but keeps the web server alive, which does NOT
# trigger a restart; that's honest (the UI stays reachable and says what
# happened) rather than flap-looping the radar. Mode starts as indoor play;
# all three modes stay live-switchable from the web UI's picker.
log "Step 9: systemd auto-start (optional)"
if ! command -v systemctl >/dev/null 2>&1; then
    warn "systemctl not found -- not a systemd machine, skipping auto-start."
else
    read -r -p "Install/refresh the openflight-lm boot service? [Y/n] " AUTOSTART
    if [ "$AUTOSTART" = "n" ] || [ "$AUTOSTART" = "N" ]; then
        warn "Skipped. Install later by re-running the wizard."
    else
        if [ -z "$CLI_PORT" ] || [ -z "$GEOM_PORT" ]; then
            warn "hardware.env has no serial ports yet -- installing anyway;"
            warn "the service will exit with a clear error until step 5"
            warn "succeeds on a re-run (plug in the radar first)."
        fi
        UNIT_PATH="/etc/systemd/system/openflight-lm.service"
        log "Writing $UNIT_PATH (needs sudo)..."
        sudo tee "$UNIT_PATH" >/dev/null <<UNIT
# openflight-lm.service -- generated by scripts/setup_wizard.sh (step 9).
# Safe to re-run the wizard: this file is re-rendered each time.
[Unit]
Description=OpenFlight LM launch monitor (IWR6843 + K-MC1)
# sound.target: the HiFiBerry capture device must exist before AudioRing
# opens it; network.target so the web UI binds cleanly at boot.
...
ExecStart=$REPO_ROOT/.venv/bin/python3 $REPO_ROOT/run_iwr6843.py --ballistics
# on-failure only: a clean operator shutdown stays down; a crash (or the
# E-7-style arg error after a hardware change) retries with backoff.
Restart=on-failure
RestartSec=5
...
UNIT
        ...
        sudo systemctl enable openflight-lm.service
        log "Enabled. Starts on next boot; start now with:"
        log "  sudo systemctl start openflight-lm"
        log "Logs: journalctl -u openflight-lm -f"
        warn "UNVERIFIED on real hardware until the Pi exists: boot ordering"
        warn "vs. the HiFiBerry overlay and USB enumeration is the bench check"
        warn "(TODO: auto-start item)."
    fi
fi
