...
if ! command -v systemctl >/dev/null 2>&1; then
    ...
    read -r -p "Install/refresh the openflight-lm2 boot service? [Y/n] " AUTOSTART
    if [ "$AUTOSTART" = "n" ] || [ "$AUTOSTART" = "N" ]; then
        ...
        UNIT_PATH="/etc/systemd/system/openflight-lm2.service"
        log "Writing $UNIT_PATH (needs sudo)..."
        sudo tee "$UNIT_PATH" >/dev/null <<UNIT
# openflight-lm2.service -- generated by scripts/setup_wizard.sh (step 9).
# Safe to re-run the wizard: this file is re-rendered each time.
[Unit]
Description=OpenFlight LM-2 launch monitor (IWR6843 + K-MC1)
...
UNIT
        ...
        sudo systemctl enable openflight-lm2.service
        log "Enabled. Starts on next boot; start now with:"
        log "  sudo systemctl start openflight-lm2"
        log "Logs: journalctl -u openflight-lm2 -f"
